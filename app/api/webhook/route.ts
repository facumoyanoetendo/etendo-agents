...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (!text.trim()) {
        responseData = { output: "Respuesta vacía del webhook" }
      } else {
        try {
          responseData = JSON.parse(text)
        } catch (parseError) {
          console.error("[v0] API Proxy - Error parseando JSON:", parseError)
      ...
      responseData = { output: textResponse || "Respuesta vacía del webhook" }
    }
